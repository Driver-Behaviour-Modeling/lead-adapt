...
class KDisksCodebook(nn.Module):
    ...
    def _compute_distances(
        ...
        """
        # Position distance
        pos_diff = deltas[:, :2].unsqueeze(1) - centroids[:, :2].unsqueeze(
            0,
        )  # [N, K, 2]
    ...
    def forward(self, deltas: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        """
        z_quantized, dictionary_loss, commitment_loss, encoding_indices = self.quantize(
            deltas,
        )
...
def create_kdisks_vocabulary(
    ...
    Returns:
        A tuple of the cluster centroids and a dict of clustering info.
    """
